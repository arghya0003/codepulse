...
export function CodeforcesDashboardClient({
  clerkId,
  clerkUser,
  profile,
  snapshots,
  ...
  // Codeforces might have specific metadata structure depending on how it's saved.
  // Using some defaults if not present
  ...
  // Codeforces specific stats if they exist in metadata, otherwise fallback to defaults
  const maxRating = metadata.maxRating || currentRating;
  const maxRank = metadata.maxRank || currentRank;
  ...
  const syncCodeforces = useCallback(async () => {
    setIsSyncing(true);
    try {
      const res = await fetch("/api/platforms/codeforces", { method: "GET" });
  ...
  const getRankColor = (rank: string) => {
    ...
    if (r.includes("legendary grandmaster") || r.includes("tourist")) return "text-red-600 font-bold";
    if (r.includes("international grandmaster")) return "text-red-500 font-bold";
    if (r.includes("grandmaster")) return "text-red-400 font-bold";
    if (r.includes("international master")) return "text-orange-500 font-bold";
    if (r.includes("master")) return "text-orange-400 font-bold";
    if (r.includes("candidate master")) return "text-violet-500 font-bold";
    if (r.includes("expert")) return "text-blue-500 font-bold";
    if (r.includes("specialist")) return "text-cyan-500 font-bold";
    if (r.includes("pupil")) return "text-green-500 font-bold";
        ...
        >
          <motion.div variants={fadeUp}>
            <h1 className="text-2xl font-bold tracking-tight">
              <span className="text-blue-600">Codeforces</span> Dashboard
            </h1>
            <p className="text-sm text-muted-foreground mt-1">
              {profile ? `@${profile.handle}` : "Connect your Codeforces account"}
            </p>
          </motion.div>
            ...
            <Card className="border-border/50 bg-card/50 h-full flex flex-col justify-center text-center p-6">
              {profile?.avatarUrl ? (
                <img
                  ...
                  alt={profile.displayName || "Codeforces"}
                  className="w-24 h-24 rounded-lg mx-auto mb-4 object-cover border border-border"
                />
              ) : (
                <div className="w-24 h-24 bg-blue-500/20 text-blue-500 rounded-lg flex items-center justify-center mx-auto mb-4 border border-blue-500/20">
              ...
              {profile?.profileUrl && (
                <a
                  ...
                  className="text-sm text-blue-500 hover:text-blue-400 flex items-center justify-center gap-2 mt-auto"
                >
                ...
                <div className="flex flex-col gap-1">
                  ...
                  {maxRating > 0 && maxRating !== currentRating && (
                    <div className="text-xs text-muted-foreground mt-4 pt-4 border-t border-border/50 flex justify-between">
                      ...
                      <span className="font-medium text-foreground">{maxRating} ({maxRank})</span>
                    </div>
                  )}
                </div>
              </CardContent>
